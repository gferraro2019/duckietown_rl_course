...
class DuckieBotAPI(object):
    ...
    def __init__(self, **params):
        ...
        self.control_time: float = params.get("control_time", 2)  # Temps de contrôle pour chaque action
        ...
        self.ACTION_MAPPING = {
            0: (1, 1),         # avancer: les deux roues avancent d'une même valeur
            1: (-1, -1),       # reculer: les deux roues reculent d'une même valeur
            2: (0.25, -0.25),  # droite: roue gauche avance, roue droite recule
            3: (-0.25, 0.25),  # gauche: roue gauche recule, roue droite avance
    ...
    def stop_robot_direct(self):
        ...
        # Réactiver le PID après l'arrêt si nécessaire
    ...
    def apply_action(self, action):
        ...
        if action in self.ACTION_MAPPING:
            ...
            self.target_position_left = self.current_ticks_left + (left_factor * self.position_increment)
            self.target_position_right = self.current_ticks_right + (right_factor * self.position_increment)
    ...
    def pid_control(self, event):
        ...
        if self.action_active:
            ...
            #     self.action_completed = True
            #     self.set_velocity_raw(0.0, 0.0)  # Arrêter les moteurs
